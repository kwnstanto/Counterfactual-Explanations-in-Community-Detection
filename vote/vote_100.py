...
file_path = r".\Datasets\vote\soc-wiki-Vote.mtx"
...
edge_data.to_csv(r'.\Datasets\vote\soc-wiki-Vote.txt', header=None, index=None, sep=' ', quoting=csv.QUOTE_NONE, escapechar=' ') 
# Create a graph using NetworkX from the rescaled edge data
G = nx.read_edgelist(r'.\Datasets\vote\soc-wiki-Vote.txt', create_using=nx.Graph())
## create file for Gephi
nx.write_gexf(G, r".\Datasets\vote\soc-wiki-Vote.gexf")
...
def sort_neighbors_dict(dct):
    return {str(key): dct.get(str(key), []) for key in range(1, 890)}
...
def endog(edges, part, num_nodes=889):
...
def find_counterf_causes_100(graph, partition, endogenous_edges, thresh):
    ...
    return counterfactuals, contingencies, cause_details, cd_method_past  
