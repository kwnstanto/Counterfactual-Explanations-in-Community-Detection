...
file_path = r".\Datasets\ucidata-zachary\out.ucidata-zachary"
...
edge_data.to_csv(r'.\ucidata-zachary(weighted)\out.ucidata-zachary_rescaled.txt', header=None, index=None, sep=' ', quoting=csv.QUOTE_NONE, escapechar=' ') 
# Create a graph using NetworkX from the rescaled edge data
G = nx.read_weighted_edgelist(r'.\ucidata-zachary(weighted)\out.ucidata-zachary_rescaled.txt', create_using=nx.Graph())
## create file for Gephi
nx.write_gexf(G, r'.\ucidata-zachary(weighted)\out.ucidata-zachary.gexf')

threshold = 0.3 # Set the threshold τ={0.3,0.5,0.8}
l=1 # set the bound of contingency set
...
file_path = r".\ucidata-zachary(weighted)"
...
def endog(edges, part, num_nodes=34):
