# -*- coding: utf-8 -*-
"""
Created on Tue Mar 11 11:06:46 2025
...
file_path = r".\Datasets\ucidata-zachary(kar)\out.ucidata-zachary"
...
edge_data.to_csv(r'.\Datasets\ucidata-zachary(kar)\out.ucidata-zachary.txt', header=None, index=None, sep=' ', quoting=csv.QUOTE_NONE, escapechar=' ') 
# Create a graph using NetworkX from the rescaled edge data
G = nx.read_edgelist(r'.\Datasets\ucidata-zachary(kar)\out.ucidata-zachary.txt', create_using=nx.Graph())
## create file for Gephi
nx.write_gexf(G, r".\Datasets\ucidata-zachary(kar)\out.ucidata-zachary.gexf")

# Set the threshold τ={0.3,0.5,0.8}
threshold=0.5
# Set the constant l={μ/2,μ,2μ}
m=round(len(G.edges)/len(G.nodes))
l=2*m-1 # minus the intervention of fixed node e [0,1,5]
...
# Compute the initial community structure between Greedy
part=Greedy(G)
...
file_path = r".\Datasets\ucidata-zachary(kar)"
...
def sort_neighbors_dict(dct):
    return {str(key): dct.get(str(key), []) for key in range(1, 35)}
...
def endog(edges, partition, num_nodes=34):
    ...
    for node1, node2 in edges:
        ...
        if partition[node1] == partition[node2]:  # Same partition (endogenous)
...
# Find the Endogenous and Exogenous set for each partition
E_e, E_x= endog(edge_list, part)
E_e = misval(E_e)

# Save the sorted dictionary
save_dict(E_e, "Endogenous_set-Greedy.json", file_path)
