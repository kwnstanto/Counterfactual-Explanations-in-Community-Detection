...
file_path = r".\Datasets\Facebook(fb-75)\socfb-American75.mtx"
...
edge_data.to_csv(r'.\Datasets\Facebook(fb-75)\socfb-American75.txt', header=None, index=None, sep=' ', quoting=csv.QUOTE_NONE, escapechar=' ') 
# Create a graph using NetworkX from the rescaled edge data
G = nx.read_edgelist(r'.\Datasets\Facebook(fb-75)\socfb-American75.txt', create_using=nx.Graph())
## create file for Gephi
nx.write_gexf(G, r".\Datasets\Facebook(fb-75)\socfb-American75.gexf")
...
# Function to transform community partition to proper dictionary
...
file_path = r".\Datasets\Facebook(fb-75)"
...
def sort_neighbors_dict(dct):
    return {str(key): dct.get(str(key), []) for key in range(1, 217663)}
...
def endog(edges, part, num_nodes=217662):
...
Counterf_causes,Cont_sets,count_causes,comm_det_method_past=find_counterf_causes_100(G,part,E_e_100, threshold)
...
# Save the sorted dictionaries
save_dict(Counterf_causes, "Counterf_causes_100.json", file_path)
save_dict(Cont_sets, "Cont_sets_100.json", file_path)
save_dict(count_causes, "Count_causes_100.json", file_path) 
...
F1,SR,total_time=F1_calc_100(G,Founded_causes,NMI_scores,time_for_countf_causes,time_for_actual_causes)
print(f'l = {l}, tau = {threshold}, F1 = {F1}, SR = {SR} and total time = {total_time}.')
save_dict(cont_all, "Actual_Causes_100.json", file_path)

"FOR RUNNING 5 EXPERIMENTS WITH 100 RANDOM NODES EACH TIME TO GET F1,SR,TIME RATIOS"
# SR_list=[]
# F1_list=[]
# t_list=[]
# for i in range(1,6):  
...
# F1_ratio,SR_ratio,t_ratio=F1_ratio/5,SR_ratio/5,t_ratio/5
